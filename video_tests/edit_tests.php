<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>Система за провеждане на тестове</title>
    <link href="css/style.css" rel="stylesheet">
</head>
<body>
<ul class="test-nav">
    <li><a href="index.php">Начало</a></li>
    <li><a href="tests.php">Тестове</a></li>
    <li><a href="edit_tests.php">Администрация</a>
    <li><a href="results.php">Резултати</a></li>
</ul>
<div class="test">
    <h1>Администрация</h1>
    <div class="dashboard-transparent">
        <div>
            <?php
            require_once('db/db_connection.php');

            $query = mysqli_query($dbc, "SELECT test_id,title FROM tests order by test_id");

            while ($row = mysqli_fetch_assoc($query)) {
                echo '<div class="test-bar"><form method="POST" action="delete.php" >
                <input type="hidden" name="name" value="' . $row['title'] . '"/>
                <a href="test_template.php?test_id=' . $row['test_id'] . '&test_name=' . $row['title'] . '">' . $row['title'] . '</a>
                        <button style="float: right; align-content: center""submit" name="testId" value="' . $row['test_id'] . '">
                        Изтрий
                        </button> <br/>
                        </form></div>';
            }

            ?>
        </div>
        <div class="create-new-button">

            <a href="create_test.html">Създай нов тест</a>
        </div>

        <div>
            <form method="POST" action="import.php">
                <button type="submit" class="insert-button">
                    Вмъкни тест
                </button>
                <input type="file" name="fileName">
                <br/>
            </form>
        </div>
    </div>
</div>
<footer>Copyright © 2021 PP Group Limited</footer>
</body>
</html>
